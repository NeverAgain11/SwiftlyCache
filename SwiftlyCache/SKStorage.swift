...
/**
 支持for...in循环
 */
public class SKStorageGenerator<Value:Codable>: IteratorProtocol {
...
public class SKStorage<Value: Codable> {
    ...
    public init(cacheName: String = "swiftyCache") {
        self.diskCachePath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0]
...
extension SKStorage: Sequence {
    ...
    /**
     返回该序列元素迭代器
     */
    public func makeIterator() -> SKStorageGenerator<Value> {
...
extension SKStorage: CacheAware {
    /**
     设置需要缓存的key和value
    ...
    @discardableResult
    public func set(object: Value?, forKey key: String, cost: vm_size_t = 0) -> Bool {
     ...
     */
    public func set(object: Value?, forKey key: String, cost: vm_size_t = 0, completionHandler:@escaping((_ key: String, _ finished: Bool) -> Void)) {
        queue.async {
